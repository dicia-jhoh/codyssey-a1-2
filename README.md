...
각 항목은 **실제 코드 원문** + **왜 그렇게 했는가** + **어디까지 확장되는가** 순입니다.
...
이 방식이 성립하려면 **세 가지 규칙을 함께 지켜야** 합니다. 하나라도 빠지면 실패가 사라집니다.

| 규칙 | 이 프로그램에서 |
|---|---|
| 빈 결과와 `errors` 항목을 **짝으로** 읽는다 | 0건과 호출 실패를 리포트에서 구분(15번) |
| 실패를 **반드시 밖으로 노출**한다 | `errors` 섹션은 비어도 "- 없음"을 찍고 지우지 않는다 |
| 누가 넣는지 **한 방향으로 고정**한다 | 각 단계는 자기 실패만 넣고 남의 항목을 읽거나 지우지 않는다 |

`errors` 리스트를 인자로 넘기는 것은 밖의 리스트를 고치는 부수효과입니다. 단계가 셋뿐이고
방향이 한쪽이라 지금은 이 편이 읽기 쉽지만, 단계가 늘거나 병렬 실행이 들어가면
`Result` 래퍼(`(값, 오류들)` 쌍을 반환)로 바꾸는 것이 맞습니다 — 바꿀 곳은 세 단계 함수의
반환 타입과 `main.run()` 의 조립부입니다.
...
더 근본적인 기준은 **HTTP 메서드의 의미론**입니다. REST 에서 메서드는 "어떻게 보내느냐"가
아니라 **"자원에 무엇을 하느냐"** 를 나타냅니다.

| 성질 | 뜻 | GET | POST |
|---|---|---|---|
| 안전성(safe) | 서버 상태를 바꾸지 않는가 | 예 | 아니오 |
| 멱등성(idempotent) | 여러 번 보내도 결과가 같은가 | 예 | 아니오 |
| 캐시 가능 | 중간 계층이 응답을 재사용해도 되는가 | 예 | 원칙적으로 아니오 |

맛집 검색은 "이 조건에 해당하는 장소 목록"이라는 **자원을 읽는** 일이라 GET 이 의미상 맞습니다.
안전·멱등하므로 프록시가 캐시해도 되고, 재시도를 넣어도 부작용이 없습니다. 반대로 LLM 호출은
호출할 때마다 **새 응답을 생성**시키는 처리 요청이라 안전하지도 멱등하지도 않습니다 —
그래서 재시도를 1회로 못 박았습니다. 길이·기록은 이 의미론이 만든 **결과**이지 원인이 아닙니다.

인증 키는 둘 다 **헤더**에 넣고 URL 에 넣지 않습니다(URL 은 로그·히스토리에 그대로 남습니다).
헤더 형식은 서비스마다 달라 Kakao 는 `KakaoAK <키>`, OpenAI 는 `Bearer <키>` 입니다.
...
**검토 후 배제한 방어**도 근거와 함께 적어 둡니다 — 안 떠올린 것과 안 쓰기로 한 것은 다릅니다.

| 방어책 | 배제 근거 |
|---|---|
| 텍스트에서 `{`~`}` 잘라내기 | 중첩 괄호·문자열 안 괄호에서 **조용히** 틀립니다. 명확히 실패하는 편이 낫습니다 |
| 정규식으로 키를 하나씩 긁기 | JSON 을 안 쓰는 것과 같아져 11번의 "결합이 문장 표현에 걸리는" 문제가 재발합니다 |
| 3회 이상 재시도 | 2회 실패는 프롬프트·모델 쪽 문제라 대기시간 대비 성공률이 낮습니다 |

**확장 경로는 정해져 있습니다.** 제공자의 구조화 출력 기능(OpenAI 의 `response_format`
= JSON mode, 또는 JSON Schema 강제)을 쓰면 파싱 실패 자체가 사라집니다 — 서버가 문법을
보장하므로 `FENCE` 도 재시도도 필요 없어집니다. 바꿀 곳은 `call_llm` 의 `body` 한 곳이고,
`parse_recommendation` 의 **키·타입 검증은 그대로 남깁니다**(문법이 맞아도 값이 틀릴 수는
있습니다). 이 미션은 요구된 표준 라이브러리 범위와 제공자 중립성을 지키려고 프롬프트 강제 +
검증 조합을 택했습니다 — 어느 LLM 으로 바꿔도 `recommend.py` 가 그대로 돕니다.

### 15. 검색 결과 0건일 때의 표기·정책

섹션을 지우지 않고 "데이터 없음" + **다음 행동**을 함께 적습니다.
...
그래서 진단(왜 비었나)과 처방(무엇을 하면 되나)을 리포트 안에서 끝냅니다.
세 상황은 이렇게 구분됩니다.
...
캐시 키는 **날짜 하나**, 무효화는 `--no-cache` 한 수단입니다. 이 미션의 산출물이 "날짜당 리포트
한 장"이라 키와 산출물이 1:1 로 맞아떨어지기 때문입니다. 무효화 정책은 **네 축**으로 확장되며,
각각 어디를 고치는지까지 정해 두었습니다.

| 축 | 언제 필요해지나 | 어떻게 | 고칠 위치 |
|---|---|---|---|
| 성공만 캐시 | 401 로 빈 맛집 결과가 굳는 것을 막을 때 | `errors` 가 비어 있을 때만 저장 | `save_results` |
| 시간 만료 | 며칠 지난 추천을 새로 받고 싶을 때 | 파일 mtime 이 N일 지나면 무시 | `load_cache` |
| 옵션 반영 | `--size` 를 바꿔 다시 뽑을 때 | 키를 `날짜+옵션 해시` 로 | 양쪽 파일명 규칙 |
| 버전 반영 | 프롬프트·스키마를 고쳤을 때 | 키에 프롬프트 버전 문자열 포함 | 양쪽 파일명 규칙 |

**적용 순서는 첫 줄부터**입니다. 실전에서 가장 먼저 아픈 것이 "실패가 캐시되는" 경우이고,
그 한 줄(`if not errors:`)이 나머지 셋보다 값싸기 때문입니다. 파일명 규칙을 바꾸는 두 축은
기존 캐시를 무효로 만들므로 함께 넣는 편이 낫습니다.

### 17. (What-if) 추천 도시가 넓게 나올 때의 입력 정규화

넓은 지명은 **세 층으로 나눠** 좁힙니다. 층을 나눈 이유는 보정의 성격이 서로 다르기
때문입니다 — "제주도→제주시"는 **표기** 문제이고, "경상북도→경주"는 **범위** 문제입니다.
한 표에 뭉뚱그리면 왜 바뀌었는지 설명할 수 없습니다.

```python
# ① 표기 흔들림 보정
CITY_ALIASES = {"제주도": "제주시", "울릉도": "울릉군", "여수시": "여수"}

# ② 광역 단위 축소 — "경상북도 맛집" 은 결과가 흩어져 쓸모가 없다
REGION_TO_CITY = {
    "강원도": "강릉", "강원특별자치도": "강릉", "경기도": "수원",
    "충청북도": "청주", "충청남도": "공주", "전라북도": "전주",
    "전북특별자치도": "전주", "전라남도": "여수", "경상북도": "경주",
    "경상남도": "통영", "제주특별자치도": "제주시",
}

# ③ 광역시는 검색은 되지만 범위가 넓다 — 대표 상권으로 좁힌다
METRO_HOTSPOT = {
    "서울": "서울 종로", "부산": "부산 해운대", "대구": "대구 중구",
    "인천": "인천 중구", "광주": "광주 동구", "대전": "대전 유성구",
    "울산": "울산 남구",
}
```

적용 순서까지 코드가 정합니다. 광역 단위가 앞에 붙은 합성형("강원도 강릉")은 먼저 뒤쪽
시/군만 남긴 뒤 나머지 표를 태웁니다.
...
실제 동작(`python -c "import places; print(places.normalize_city('경상북도'))"`):

```text
'제주도'      -> 제주시      # ① 표기
'경상북도'    -> 경주        # ② 범위
'강원도 강릉' -> 강릉        # 합성형 → 뒤쪽 시/군
'부산'        -> 부산 해운대  # ③ 광역시 상권
'통영'        -> 통영        # 이미 좁다 — 건드리지 않는다
```

이 세 층은 **호출 수를 늘리지 않으면서** 검색 품질을 올립니다(문자열 치환뿐). 규모가 커지면
다음 두 층이 이어집니다: **질의 분해**(`부산 서면`·`부산 해운대` 를 각각 검색해 합침 — 호출 수가
도시당 N배) 와 **좌표 기반 반경 검색**(지명을 좌표로 바꾼 뒤 반경 질의 — 표기 흔들림에서 완전히
자유로워짐). 확장 지점도 정해져 있습니다: `places.py` 에 `_geocode(city)` 를 더하고
`search_places` 가 좌표를 받도록 바꾸면 되며, `report.py`·`main.py` 는 그대로입니다.

호출 비용이 0인 층을 먼저 전부 쓰고, 비용이 붙는 층은 필요할 때 여는 순서입니다.
