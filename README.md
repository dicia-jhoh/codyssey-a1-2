...
[사용자] -date 2026-09-15
    ↓
[1단계] LLM API ─────→ {"recommended_city": "강릉", "weather": …, "events": […], "reason": …}
    ↓ recommended_city 값을 그대로 전달
[2단계] 지도 API ────→ [{"name": "초당순두부마을", "address": …, "url": …}, …]  (0건 가능)
    ↓ 둘 다 전달
...
필요한 필드(상호·주소·분류·링크·좌표)를 전부 줍니다. Naver Local Search 로 바꾸려면
`places.py` 의 `search_places()` 와 `_to_item()` 두 함수만 고치면 됩니다 — 응답 필드를 우리 형식으로
한 번 옮겨 두었기 때문에 리포트 코드는 건드릴 필요가 없습니다.
...
```python
# 1단계가 돌려준 것 (dict)
{"recommended_city": "강릉", "weather": "...", "events": [...], "reason": "..."}

# 2단계는 이 한 줄로 이어받는다
restaurants = places.find_restaurants(map_key, recommendation["recommended_city"], errors)
```
...
| 파일 | 내용 |
|---|---|
| `results/2026-09-15.json` | 원본 데이터 — `recommendation`(1차 JSON) · `restaurants`(리스트, 0건 가능) · `errors`(배열) |
...
**맛집이 0건이고 지도 API 가 실패한 경우**(같은 데모의 사례 2):
...
> 보너스 1(복수 지역 추천)은 `recommended_city` 를 `recommended_cities` 배열로 바꾸고
> 2단계를 반복문으로 감싸면 됩니다 — 현재 구조에서 `recommend.REQUIRED_KEYS` 와
> `main.run()` 의 2단계 블록만 손대면 되도록 설계돼 있습니다.
